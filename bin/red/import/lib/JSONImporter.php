...
//			if (!$this->site->isUser($user->guid)) {
	...
	private function importGroup($object) {
		...
		// Importa campos personalizados para grupos
		foreach ($metadata as $key => $value) {
			if ($key == 'owner_guid') {
				if ($value == 0) {
					$group->$key = $this->getDefaultOwner();
				} else {
					$group->$key = $this->translate($value);
				}
		...
		// Desactiva herramientas de grupos que no usamos
		$group->activity_enable = 'no';
		$group->tp_images_enable = 'no';
		$group->membersmap_enable = 'no';
		$group->related_groups_enable = 'no';
